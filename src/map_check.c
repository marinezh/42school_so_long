	...
	while (map->area[x])
	{
		...
		while (map->area[x][y])
		{
			...
			if (c != '0' && c != '1' && c != 'C' && c != 'E' && c != 'P'
				&& c != 'M')
				return (error_msg("Error: Wrong symbol found"), -1);
			y++;
		}
		x++;
	}
...
int	borders_check(t_map *map)
{
	...
	{
		if (map->area[0][col] != '1')
			return (error_msg("Error: Top border is not closed"), -1);
		col++;
	}
	col = 0;
	while (map->area[map->size_y - 1][col])
	{
		if (map->area[map->size_y - 1][col] != '1')
			return (error_msg("Error: Bottom border is not closed"), -1);
		col++;
	}
	row = 0;
	while (map->area[row])
	{
		if (map->area[row][0] != '1' || map->area[row][map->size_x - 1] != '1')
			return (error_msg("Error: Side borders are not closed"), -1);
		row++;
	}
	return (1);
}
